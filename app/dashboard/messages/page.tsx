...
export default function MessagesPage() {
  ...
  if (!authenticated) return <section className="auth-required"><div className="auth-required-icon">◌</div><h1>Les échanges, en toute simplicité.</h1><Link className="btn btn-ink" href="/auth?mode=login">Se connecter</Link><Link className="auth-required-link" href="/explorer">Explorer les logements →</Link></section>;

  return <section className="dashboard-screen messages-screen"><div className="screen-heading"><div><h1>Messages</h1></div><Link className="btn btn-ink" href="/explorer">Explorer</Link></div><div className="messages-layout"><aside className="conversation-list"><div className="conversation-list-head"><strong>Conversations</strong><span>{conversations.length}</span></div><label className="conversation-search"><span>⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Rechercher" /></label>{filtered.map((conversation) => <button className={`conversation-item ${conversation.id === active.id ? "active" : ""}`} type="button" key={conversation.id} onClick={() => setActiveId(conversation.id)}><Image src={conversation.image} alt="" width={42} height={42} /><span className="conversation-copy"><strong>{conversation.name}</strong><small>{conversation.property}</small><em>{conversation.preview}</em></span><span className="conversation-meta"><small>{conversation.time}</small>{conversation.unread > 0 && <b>{conversation.unread}</b>}</span></button>)}</aside><section className="chat-panel"><header className="chat-head"><Image src={active.image} alt="" width={44} height={44} /><div><strong>{active.name}</strong><span>{active.online ? "En ligne" : "Propriétaire ICIMO"}</span></div><button type="button" aria-label="Plus d'options">•••</button></header><div className="chat-property">{active.property}<Link href="/explorer">Voir le logement →</Link></div><div className="chat-messages">{active.messages.map((message, index) => <div className={`chat-message ${message.from}`} key={`${message.time}-${index}`}><p>{message.text}</p><small>{message.time}</small></div>)}</div><form className="chat-compose" onSubmit={sendMessage}><input value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Écrire un message..." /><button type="submit" aria-label="Envoyer">↑</button></form></section></div></section>;
}
